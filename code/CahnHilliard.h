...
public:
    ...
    void dump(const std::string fname, int it_num);
    ...
    void integrate()
    {
        ...
        while (elapsed < duration) {
            ...
            // swap data arrays, MPI output data and advance time
            std::swap(u_, utmp_);
            std::swap(derivative_mu_, derivative_mu_tmp_);
            dump("steps/step_" + std::to_string(it_num) +".bin", it_num);
        }
    }
...
private:
    ...
    const int nodes_;
    int p_, stride_;
    double *u_, *utmp_, *derivative_mu_, *derivative_mu_tmp_; // species u, derivative of chemical potential (including ghosts, per rank)
    ...
    // For 13-point stencil to calculate biharmonic operator we need to send corner elements to diagonal neighbors
    ...
    {
        // Calculate time difference for concentration u
        ...
        // Calculate derivative of chemical potential for next iteration.
